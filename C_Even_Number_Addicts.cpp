...
bool solve(ll x, ll y, ll sum)
{
    ...
    bool op1 = false;
    if (x - 1 >= 0 && y - 1 >= 0)
        op1 |= solve(x - 1, y - 1, sum ^ 1) == false;
    if (x - 2 >= 0)
        op1 |= solve(x - 2, y, sum ^ 1) == false;
    bool op2 = false;
    if (x - 1 >= 0 && y - 1 >= 0)
        op2 |= solve(x - 1, y - 1, sum) == false;
    if (y - 2 >= 0)
        op2 |= solve(x, y - 2, sum) == false;
    return op1 || op2;
}
void run_case()
{
