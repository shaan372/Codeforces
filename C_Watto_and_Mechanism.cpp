...
Node *insert(Node *node, string &s, ll ind)
{
    if (ind == s.length())
    {
        node->flag = true;
    ...
    if (node->links[s[ind] - 'a'] == NULL)
        node->links[s[ind] - 'a'] = new Node();
    node->links[s[ind] - 'a'] = insert(node->links[s[ind] - 'a'], s, ind + 1);
    return node;
}
bool search(Node *node, string &s, ll ind, bool flag)
{
    if (ind == s.length())
    {
        if (flag == true && node->flag == true)
            return true;
        return false;
    }
    if (flag == false)
    {
        for (char i = 'a'; i <= 'c'; i++)
        {
            if (s[ind] != i && node->links[i - 'a'] != NULL)
            {
                if (search(node->links[i - 'a'], s, ind + 1, true) == true)
                    return true;
            }
        }
    }
    if (node->links[s[ind] - 'a'] == NULL)
        return false;
    return search(node->links[s[ind] - 'a'], s, ind + 1, flag);
}
int main(int argc, char const *argv[])
{
    ...
    for (ll i = 0; i < n; i++)
    {
        string s;
        cin >> s;
        node = insert(node, s, 0);
    }
    for (ll i = 0; i < m; i++)
    {
        string s;
        cin >> s;
        if (search(node, s, 0, false) == true)
            cout << "YES" << nl;
        else
            cout << "NO" << nl;
    }
